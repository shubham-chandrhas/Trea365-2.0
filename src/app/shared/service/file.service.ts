...
export class FileService {
  ...
  formDataAPICall(FormDataObj: any, url: string, callback) {
    const self = this;
    const tokan = localStorage.getItem("KEY")
    ...
    this._http
      .post(self.config.apiEndpoint + url, FormDataObj, options).pipe(
      ...
      .subscribe(
        ...
        error => {
          if (error.status === 401) {
            ...
            const errorStatus: any = JSON.parse(error._body);
            ...
            typeof errorStatus.message === "string"
              ? ""
              : (error.message) ? error.message : (error.message = self.constant.EXCEPTIONAL_MSG);
    ...
    if (
      ...
      self.util.initiateAuth(function(
        ...
        if (cognitoError) {
          ...
          self._http
            .post(url, requestParam, options).pipe(
            map(res => res.json()))
            .subscribe(
              data => {
                return callback(false, data);
