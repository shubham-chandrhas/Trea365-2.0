...
export class ProductReviewComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.currentURL == 'product-review' ? this.util.menuChange({'menu':3,'subMenu':19}) : '';
        this.routeObj = { 'list': '/inventory/csa/product-list/0', 'add': '/inventory/csa/add-product' };
        this.productInfo = JSON.parse(sessionStorage.getItem('productInfo'));
        console.log(this.productInfo);
        for(let i=0; i<this.productInfo.requestData.locations.length; i++){
            this.totalQuantity = this.totalQuantity + parseFloat(this.productInfo.requestData.locations[i].quantity);
            this.totalPrice = this.totalPrice + parseFloat(this.productInfo.requestData.locations[i].purchase_price);
        }
    }
    ...
    submitProduct() {
        try {
            ...
            for (var i = 0; i < reqObj.locations.length; ++i) {
                delete reqObj.locations[i].location_name;
                delete reqObj.locations[i].location_id;
                delete reqObj.locations[i].location_tag_name;
                delete reqObj.locations[i].locationList;
                delete reqObj.locations[i].locationTagList;
                totalprodQty = parseFloat(totalprodQty) + parseFloat(reqObj.locations[i].quantity);
            }
