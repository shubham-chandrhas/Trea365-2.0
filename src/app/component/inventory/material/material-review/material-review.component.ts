...
export class MaterialReviewComponent implements OnInit {
  ...
  ngOnInit() {
      ...
      this.currentURL == 'material-review' ? this.util.menuChange({'menu':3,'subMenu':34}) : '';
      this.routeObj = { 'list': '/inventory/csa/material-list/0', 'add': '/inventory/csa/add-material' };
      this.materialInfo = JSON.parse(sessionStorage.getItem('materialInfo'));
      console.log("this.materialInfo",this.materialInfo);
      for(let i=0; i<this.materialInfo.requestData.locations.length; i++){
        this.totalQuantity = this.totalQuantity + parseFloat(this.materialInfo.requestData.locations[i].quantity);
        this.totalPrice = this.totalPrice + parseFloat(this.materialInfo.requestData.locations[i].purchase_price);
      }
  }
    submitMaterial() {
        try {
            ...
            this.http.doPost('material/create', reqObj, function (error: boolean, response: any) {
