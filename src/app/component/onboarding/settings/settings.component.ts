...
export class SettingsComponent implements OnInit {
  ...
  public editSettingsForm(option,settingData: any = {}) {
    this.editSettingsFrm = this.fb.group({
      ...
      invoice_midsection: new FormControl(option == "0" ? "" : settingData.setting_key == 'invoice_midsection' ? settingData.setting_value : ''),
    });
  ...
  get invoice_midsection() {
    return this.editSettingsFrm.get("invoice_midsection");
  }
        ...
        this.file.formDataAPICall(formData, "setting/update", function(
          ...
          if (error) {
            ...
            self.errMsg = response.message;
              ...
              if(response.data){
                    let preData = JSON.parse(atob(localStorage.getItem("USER")));
                    preData.settings = [];
                    preData.settings = response.data;
                    localStorage.setItem("USER", btoa(JSON.stringify(preData)));
