...
export class WorkOrderDialog {
  ...
  constructor(
    ...
    if(dataObj && dataObj.pmObj)
    {
        this.inventoryObj = dataObj.pmObj;
        this.selectedWO = dataObj.selectedWO;
    }
...
get approve_note() {
      ...
      confirm(): void {
        ...
        try {
            ...
            this.http.doPost("work-order/update/status", reqObj, function (error: boolean, response: any) {
    ...
    editPM(): void {
        ...
        self.closeDialog();
        self.util.changeEvent({
                'source': 'WORKORDER_EDIT',
                'action': 'EDIT_PM',
                'data': self.selectedWO
            });
    }
  ...
  quotationList() {
    ...
    try {
      this.http.doGet("quotation/list/wo", function(
        error: boolean,
        response: any
      ) {
        if (error) {
