...
export class WOServicesComponent implements OnInit {
    ...
    setServicesData(): void {
        if(this.WOService.WO_DATA.servicesDetails || this.WOService.WO_DATA.wo_services){
            ...
            // console.log(this.WOService.WO_SETUP_DATA);
            // this.addExWOServicesForm('1', this.WOService.WO_SETUP_DATA);
            this.action = 'ADD';
        }else{
            this.addServicesForm('0');
            this.action = 'ADD';
        ...
        try {
            ...
            if (option == '1') {
                for (var i = 0; i < data.services.length; i++) {
                    if(parseInt(data.services[i].quantity) > 0){
                ...
                if(parseInt(data.services[i].quantity) > 0)
                {
                    ...
                    this.servicesForm.get('woRemainingQuantity').setValue(woRemainingQuantity);
                        ...
                        this.services.push(this.fb.group({
                            ...
                            service_definition: new FormControl(option == '1' ? data.services[i].service_definition : '', []),
                            service_definition_id: new FormControl(option == '1' ? data.services[i].service_definition_id : ''),
                            ad_hoc_service: new FormControl(option == '1' ? data.services[i].service_definition_id == 0 ? data.services[i].service_definition : '' : ''),
