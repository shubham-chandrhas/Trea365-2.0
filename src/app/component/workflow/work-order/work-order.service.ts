...
export class WorkOrderService {
    ...
    checkValidation(woData, callback){
        ...
        if(woData.currentAction == 'EDIT'){
            for(let i = 0; i < this.WO_DATA.materialsDetails.length; i++){
                if(woData.isEdit && i < this.WO_DATA.materialsDetails.length){
                    this.WO_DATA.materialsDetails[i].wo_material_id = this.WO_DATA.materialsDetails[i].wo_material_id;
                }
            }
        }else{
            for(let i = 0; i < this.WO_DATA.materialsDetails.length; i++){
                if(woData.isEdit && i < woData.WOSubContractorObj.materialsDetails.length){
                    this.WO_DATA.materialsDetails[i].wo_material_id = woData.WOSubContractorObj.materialsDetails[i].wo_service_id;
                }
            }
        }
        ...
        if(woData.currentAction == 'EDIT'){
            for(let i = 0; i < this.WO_DATA.servicesDetails.length; i++){
                if(woData.isEdit && i < this.WO_DATA.servicesDetails.length){
                    this.WO_DATA.servicesDetails[i].wo_service_id = this.WO_DATA.servicesDetails[i].wo_service_id;
                }
            }
        }else{
            for(let i = 0; i < this.WO_DATA.servicesDetails.length; i++){
        ...
        this.WO_DATA.requirements = woData.woForm.value.requirements;
        ...
        this.WO_DATA.work_location_id = woData.selectedQuotation.work_location_id;
