...
export class ScheduleDialogComponent {
  ...
  constructor(
    ...
    this.assetList =
      this.dialog.action == "CONFIRMATION_WITH_WARNING" &&
      this.dialog.data.type.toLowerCase() == "person"
        ? JSON.parse(
    ...
    //     ? this.dialog.data.details.persons
    //     : [];
    ...
    this.assetList =
      ...
      this.dialog.data.type == "Person"
        ...
        : this.assetList;
    this.personList =
      (this.dialog.action == "CONFIRMATION_WITH_WARNING_2" || this.dialog.action == 'CONFIRMATION_WITH_WARNING') &&
      this.dialog.data.type == "Asset"
        ? JSON.parse(
            JSON.stringify(this.dialog.data.details.persons.slice(0, 4))
          ) : this.dialog.data.details.persons
          ? this.dialog.data.details.persons.slice(0, 4)
            : [];
  ...
  deleteSchedule(option): void {
    try {
      ...
      this.http.doPost("workflow/schedulings/delete", requestObj, function(
        error: boolean,
        response: any
      ) {
