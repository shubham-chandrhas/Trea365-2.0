...
export class QuotationReviewComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.isEdit = this.quotationDetails.project_estimate_id ? true : false;
        ...
        if(this.quotationDetails.clientDetails.billingLocationSameAsWork == true){
        ...
        {   
            this.quotationDetails.materialsDetails.materials.map((material) => { 
                if(material.is_delete == 0) { 
                    costOfMaterial += parseFloat(material.total_amount)
                } 
            });
        }
        ...
        {
            this.quotationDetails.servicesDetails.services.map((service) => { 
                if(service.is_delete == 0) {
                    costOfService += parseFloat(service.total_amount) 
                }
            });    
        }
        ...
        // let taxAmount = ((subTotal+adjustment)*(tax/100));
        ...
        this.quotationDetails.paymentScheduleDetails.date_items.map(item => {
            if(item.is_delete == 0) {
                this.totalScheduledPay += parseFloat(item.amount);
            }
        });
...
var randomNo = JSON.parse(sessionStorage.getItem('quotationDetails')).pe_random_number;
                  ...
                  self.PEService.updateFormStatus('scheduleFm', false);
                  //self.util.showDialog(DialogComponent, response.message, ['/workflow/quote/csa/quotation-list/0']);
                  //self.router.navigate(['/workflow/quote/csa/quotation-list/'+JSON.parse(sessionStorage.getItem('quotationDetails')).project_estimate_id]);
