...
export class NewEmployeeComponent implements OnInit, OnChanges {
    ...
    ngOnInit() {
        ...
        try {
            ...
            if (sessionStorage.getItem("emp")) {
                ...
                const editData = JSON.parse(sessionStorage.getItem("emp"));

                this.pageData.action =  editData.id ? "edit" : "add";
                 editData.mobile_no = editData.mobile_no && editData.mobile_no.length > 10 ?
                 editData.mobile_no.replace(/\D/g, "").slice(-10) :  editData.mobile_no;
                ...
                const editDays = editData.days_off;

                this.addDaysOff();

                // Before
                this.setDaysOff(editDays);
    ...
    createForm(): void {
        ...
        this.addEmpFm = this.fb.group({
            ...
            username: new FormControl("", [
                ...
                CustomValidator.checkAvaibility(this.hr, 'username', {
                    user_id:
                    this.pageData.action == "add"
                        ? ""
                        : JSON.parse(sessionStorage.getItem("emp")).id
            ...
            email_id: new FormControl("",
                ...
                CustomValidator.checkAvaibility(this.hr, 'email', {
                    user_id:
                    this.pageData.action == "add"
                        ? ""
                        : JSON.parse(sessionStorage.getItem("emp")).id
            ...
            wage_amount: new FormControl(0.00, [
                Validators.min(0),
            ...
            permission_role_id: new FormControl("",[Validators.required]),
    ...
    /**
     * Get permission_role_id field value
     * @return string
     */
  ...
  getPermissionList(): void {
    let self = this;
    try {
      this.http.doGet("permission/roles", function(error: boolean, response: any) {
        if (error) {
  ...
  getSelectedPermission(permission, event: any): void {
    if (event.isUserInput) {
      ...
      this.permission_role_id.setValue(permission.permission_role_id);
    }
  }
  ...
  public validatePermission(event: any) {
    try {
      ...
      if (match.length > 0) {
        this.permission_role_id.setValue(match[0].permission_role_id);
