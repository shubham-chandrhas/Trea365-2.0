...
export class LocationReviewComponent implements OnInit {
    ...
    try {
      if (this.router.url.split('/')[2] == 'csa-onboarding') {
        ...
        this.routeObj = {
          ...
          'add': '/admin/csa-onboarding/add-location'
        }
      } else {
        this.util.menuChange({
          'menu': 2,
        ...
        this.routeObj = {
          'list': '/admin/csa/location-list/0',
          'add': '/admin/csa/add-location'
        }
      }
      this.locationObj = JSON.parse(sessionStorage.getItem('locationDetails'));

      this.locationObj ? this.locationObj.tagSample = this.createTagSample(this.locationObj) : "";
  ...
  createTagSample(location) {
    try {
      if (location) {
        let nameArr = location.locationName.split(' '),
          tag: string = location.shortForm;
  ...
  saveLocation() {
    ...
    try {
      ...
      for (let i = 0; i < reqObj.divisions.length; i++) {
        ...
        if (reqObj.divisions[i].divisionType == 'Named') {
          for (let j = 0; j < reqObj.divisions[i].values.length; j++) {
            divObj.divisionValue.push({
              'value': reqObj.divisions[i].values[j].value
            });
          }
        }
      ...
      this.location.addLocation(reqObj, (error: boolean, response: any) => {
        self.util.removeSpinner('submit-btn', "Submit");
        if (error) {
          self.errMsg = response.message;
          self.isError = true;
          this.global.addException('location review', 'saveLocation()', response);
        } else {
