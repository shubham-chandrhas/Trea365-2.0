...
export class LocationDialog{
  ...
  updateTags(): void {
    ...
    try {
      ...
      this.http.doPost("admin/location/tag/edit", { "tagObj": updatedTagList }, (error: boolean, response: any) => {
        self.util.removeSpinner("update-record-btn", "Update");
        if (error) {
          self.isError = true;
          self.errMsg = response.message;
          this.global.addException("location", "updateTags()",response);
        }else {
          self.admin.updateList(true);
          self.util.setPopupFlag(false);
          // console.log(self.currentPath);
                    self.currentPath === "location-list" ?
                    (self.isSuccess = true, self.successMsg = response.message) : self.dialogRef.close();
