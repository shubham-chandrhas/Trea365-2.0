...
export class AdminService {
  ...
  updateList(newRecord: any) {
    this.adminSource.next(newRecord);
  }
  deleteRecordFromList(recordObj: any) {
    this.adminDeleteSource.next(recordObj);
  }
  ...
  getManufacturerList(callback) {
    this.http.doGet("manufacturer/list", function (error: boolean, response: any) {
      return callback(error, response);
    });
  }
