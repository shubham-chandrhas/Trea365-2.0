...
export class SetPasswordStepOneComponent implements OnInit{
    ...
    public createForm() {
        const auth = this.util.getAuthData();
        this.setPassFm = this.fb.group({
            username: new FormControl(auth ? auth.Username : '', [ Validators.required ]),
            password: new FormControl(auth ? auth.Password : '', [ Validators.required ])
        });
        this.setPass(auth);
    }
    setPass(auth) {
      setTimeout(() => {
          this.setPassFm.patchValue({
        ...
        if(form.valid){
            if (form.value.new_password != form.value.confirmPassword) {
                ...
                self.cognitoService.setNewPassword(self.data, function(error: boolean, response: any){
                    self.util.removeSpinner('login-fm-btn', "Reset Password");
                    if( error ){
                        self.isError=true;
                        self.errMsg = response.message;
                    }else{
